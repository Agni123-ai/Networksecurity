...
class DataValidation:
  ...
  def isnumerical_column_exist(self,dataframe:pd.DataFrame)->bool:
    try:
      numerical_columns=self._schema_config["numerical_columns"]
      for num_col in numerical_columns:
        if num_col not in dataframe.columns:
          logging.info(f"Numerical column: {num_col} is not present in the dataframe")
          return False
  ...
  def initate_data_validation(self)->DataValidationArtifact:
    try:
      ...
      ## Reading the data from the train and test
      ...
      status=self.isnumerical_column_exist(train_dataframe)
      if not status:
        error_message=f"Train dataframe does not contain all numerical columns.\n"
      status=self.isnumerical_column_exist(test_dataframe)
      if not status:
